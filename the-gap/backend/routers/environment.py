...
async def set_location(body: LocationRequest, user_id: str = Depends(get_current_user_id)) -> JSONResponse:
    if not body.home_address.strip():
        raise HTTPException(status_code=400, detail="Home address is required.")

    saved = await save_user_location(user_id, body.home_address.strip(), body.work_address.strip())
    if not saved:
    ...
    days_stored = await fetch_and_store_weather(user_id, location["home_lat"], location["home_lon"])
...
@router.get("/location")
async def get_location(user_id: str = Depends(get_current_user_id)) -> JSONResponse:
    location = await get_user_location(user_id)
    if not location:
...
@router.post("/sync")
async def sync_environment(user_id: str = Depends(get_current_user_id)) -> JSONResponse:
    """
